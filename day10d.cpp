...
void solve() {
    ...
    //v.push_back(s);
    if((m[a.first][a.second] == 'L') || (m[a.first][a.second] == '7') || (m[a.first][a.second] == 'J') || (m[a.first][a.second] == 'F')) {
        //vert++;
        v.push_back(a);
        //area += (a.second*i - a.first*j);
        //cout << area << " ";
    }
    ...
    while((a.first != s.first) || (a.second != s.second)) {
        ...
        if((m[a.first][a.second] == 'L') || (m[a.first][a.second] == '7') || (m[a.first][a.second] == 'J') || (m[a.first][a.second] == 'F')) {
            ...
            if(v.size() != 1) {
                area += (a.second)*v[vert].first - (a.first)*v[vert].second; //Shoelace Algorithm
                cout << area << " ";
                vert++;
            }
        }
    ...
    area += (v[0].second)*v[vert].first - (v[0].first)*v[vert].second;
    cout << endl << "Puntos enteros en la frontera del poligono: " << k << endl;
    area = abs(area)/2; //Shoelace Algorithm
    cout << "Vertices: " << vert << endl;
    cout << "Area: " << area << endl;
    long pdentro = area - k/2 + 1; //Pick's Theorem
    cout << "Puntos dentro: " << pdentro << endl;
